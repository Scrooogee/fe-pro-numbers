...
export const minMaxMultiply = (minFirst, minSecond, maxFirst, maxSecond) => {
    ...
    const maxNumnber = Math.max(maxFirst, maxSecond)
    return minNumber * maxNumnber
};
...
export const getFixedValue = (number, accuracy) => {
    const afterComa = `1e${accuracy}`
    return Math.round(number * +afterComa) / +afterComa
};
